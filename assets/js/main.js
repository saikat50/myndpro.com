...
(function($) {
    ...
    $(document).on('ready', function() {
        ...
        let homeUrl = $(".home-url").attr("href");
        ...
        if(homeUrl == pageURL || parts.length > 1){
            let child = document.getElementById("nav_cus").children;
    
            for(let i=1;i<child.length;i++){

                let url = $(child[i].getElementsByTagName('a')).attr('href');

                let parts = url.split("#");

                $(child[i].getElementsByTagName('a')).attr('href','#' + parts[1]);
